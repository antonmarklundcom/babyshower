...
const faces = `@font-face{font-family:'Instrument Serif';src:url(data:font/woff2;base64,${font('instrument-serif-400.woff2')}) format('woff2')}@font-face{font-family:'DM Sans';font-weight:500;src:url(data:font/woff2;base64,${font('dm-sans-500.woff2')}) format('woff2')}`;
const esc = value => value.replace(/[&<>"']/g, c => ({ '&': '&amp;', '<': '&lt;', '>': '&gt;', '"': '&quot;', "'": '&#39;' })[c]);
const browser = await chromium.launch({ executablePath: process.env.CHROME_PATH || 'C:/Program Files/Google/Chrome/Application/chrome.exe', headless: true });
try {
  const page = await browser.newPage({ viewport: { width: 1200, height: 630 }, deviceScaleFactor: 1 });
  await page.setContent(`<!doctype html><html lang="es"><meta charset="utf-8"><style>${faces}
    *{box-sizing:border-box}html,body{margin:0;width:1200px;height:630px;overflow:hidden}
    body{color:#fff;background:#2b2430;font-family:'DM Sans',Arial,sans-serif;font-weight:500}
    ...
    h1{margin:auto 0;font-family:'Instrument Serif',Georgia,serif;font-size:70px;font-weight:400;line-height:1.08;letter-spacing:-1.5px}
  </style><body><img class="photo" alt="" src="data:image/webp;base64,${photo}"><div class="scrim"></div><main><p class="brand">${esc(SITE.name)}</p><h1>${esc(PAGES['/'].h1)}</h1></main></body></html>`, { waitUntil: 'load' });
