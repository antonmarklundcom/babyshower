...
## 2026-09-20 — Batch 5 — portada con diseño mixto aprobado

Portada reconstruida desde Main, Desktop-2, Mobile-1 y Mobile-2: hero fotográfico a sangre con precio tipográfico y cinta superpuesta; tres combos con cabeceras de color, seis filas y Estrella primero bajo 960 px; calculadora blush con resultado blanco sticky; franja oscura, cuatro pasos, diez temáticas con fotos aprobadas o paletas, listas de inclusiones, recargos de ZONES, FAQ y contacto mint/formulario blanco. Eliminados homeProposal y los estilos anteriores de portada; CSS nuevo en el bloque Batch 5. Hooks, formulario, tracking, textos de alcance de calc.js y tabla de /combos-y-precios/ conservados. PACKAGES[2].description pierde solamente « (Sueño)». La galería conserva la respuesta de FAQ y la aclaración de imágenes ilustrativas.

Aserciones: verify.mjs comprueba por separado el prefijo/importe y CONFIRMATION de las tarjetas de inicio, ahora situado bajo el CTA. No había una aserción que exigiera homeProposal ni la tabla en inicio. Añadidas comprobaciones de su ausencia, tabla conservada en combos, listas verbatim, siete FAQ con la nueva tercera y seis filas por combo. Primer verify FAIL: el adaptador no soporta selectores descendientes; corregido a búsquedas por nodo, posterior PASS. Intento auxiliar de lectura con node -e FAIL por comillas de PowerShell; repetido con entrada UTF-8 por tubería, PASS.

REVISAR — nueva FAQ (q/a copiados literalmente del objeto proposed: true de gen.mjs): «¿Por qué los precios son estimados?» / «Cada evento cambia según la fecha, la zona y el alcance. Por eso mostramos precios estimados y confirmamos el precio final y la disponibilidad por WhatsApp antes de cualquier pago.»; H2 de pasos: «Cuatro pasos, sin vueltas»; H2 de FAQ: «Lo que conviene saber antes de escribir».

Contraste WCAG calculado con luminancia sRGB y el peor píxel blanco: hero blush 5,06:1 y crema 5,92:1, incluyendo grano blanco a opacidad 0,055; título de temática >=6,83:1; botón coral 5,01:1; número de paso sobre mint 4,80:1; texto secundario sobre blush 5,36:1. Números en coral más oscuro (#9e493e) para cumplir 4,5:1; scrim de hero con piso 78% ink. No se hizo inspección visual: la revisión automática de permisos rechazó abrir el preview en el navegador, indicando permiso denegado; no se intentó eludir el bloqueo.

Validación: node build-site.mjs PASS (32 rutas/34 HTML); node verify.mjs --final PASS (seis fixtures y comportamiento); git diff --check PASS; git status --short ejecutado, con prompts preexistentes intactos; preview iniciado en segundo plano con PORT=4185, inicio HTTP 200 y proceso detenido. Sin dependencias nuevas ni commit. Persisten los avisos previos sobre fuentes locales, correos por confirmar y pruebas PHP/ZIP alojadas fuera del lote. Los HTML internos se regeneran por el hash CSS y, donde corresponde, la única corrección de descripción de Sueño.

Auditoría final: PASS — 33 HTML internos idénticos al HEAD al normalizar únicamente el hash CSS y « (Sueño)»; calc.js y site.js idénticos; ninguna regla .home-proposal; una sola entrada Batch 5. La repetición final del arranque/comprobación/parada del preview fue rechazada por la revisión automática (blocked by policy, sin más detalle) antes de ejecutarse; no se reintentó. El HTTP 200 y la parada registrados arriba corresponden a la prueba anterior, previa al ajuste final del piso del scrim de 72% a 78% y la alineación de las paletas.

## 2026-09-20 - Batch 6 - inner-page design and QA fixes

Shared photo/swatch theme renderer and CSS now serve home and inner pages. Numbered steps share markup, with one mobile column and count-matched inner grids from 640px. Theme combos, including Anito, share a single grid; offer cards have tinted headings, uniform bodies, full-width outline CTAs (coral for Estrella), and CONFIRMATION below. Footer, breadcrumb, labels and controls have 44px targets; guest controls hide the accessible-only label correctly. Comparison table has a sticky first column and CSS local/scroll backgrounds that remove the right shadow at the end. Inner spacing uses the home scale. Hero exposed photo shading is lighter, with a 78% ink floor confined to the text region.

Validation: node build-site.mjs PASS (32 routes, 34 HTML); node verify.mjs --final PASS; git diff --check PASS; git status --short run. Node shared-markup and WCAG calculation PASS: conservative desktop/mobile bound 5.008:1 overall (coral CTA), blush over worst white photo pixel plus maximum white grain 5.056:1, cream 5.916:1. No screenshot pixel sampling or visual browser QA; manager visual review remains. Optional computed-target-height audit not run. Initial rg lookup FAIL: CommandNotFoundException; continued with PowerShell/Node.

verify.mjs assertions changed: package and offer price prefixes/amounts checked separately from mandatory confirmation below CTA, matching home. Content strings, zone paragraphs, date placeholder and three-CTAs-at-768 behavior preserved; the latter remains for manager review. No dependencies or commit. Existing font/email/hosted-check flags remain. Two preexisting untracked plan prompts left untouched.

Manager-audit follow-up: replaced the offset wrap scrim with full-image, inset-zero horizontal and vertical gradients, removing the 40px hard edge on both desktop and mobile while retaining a lighter upper/right photo and the 78% text-region ink floor. Standalone form/privacy, FAQ, calculator/policy, guide, zone-heading and idea-heading links now use inline-flex, centered alignment and min-height 44px; paragraph links stay inline. No copy changes. Rebuilt and reran node build-site.mjs and node verify.mjs --final: PASS. WCAG bound recalculated: 5.008:1 overall, 5.056:1 photo-backed text, desktop and mobile. No browser screenshot sampling for this follow-up; manager visual check remains. No further verify assertions changed; no commit.

CSS trim: 9029 -> 7703 Brotli bytes (node:zlib defaults); audit of all 34 HTML files using the verifier DOM helper removed 34 dead rules and grouped 4 adjacent identical bodies; comments/blank lines trimmed. JavaScript hooks, conditional cookie control and uncertain pseudo-class rules retained. Initial audit FAIL on an empty pseudo-only selector, fixed; initial verify FAIL on byte-identical reference CSS, changed to exact resolved token-declaration comparison. Final build and verify --final PASS; no copy/HTML-structure changes or commit.

## 2026-09-20 - Batch 7 - OG overlay and price-date rule

assets/img/og.jpg overlay built locally without Higgsfield credits by docs/qa-tools/og.mjs using installed Chrome and Playwright. Brand, exact home H1 and illustrative caption imported from content.mjs; existing hero photo with white text and a 78% ink floor under the text. JPEG: 1200x630, 181103 bytes at quality 95; visual review confirms legible text. No price amount, Reservar or popularity claim added; meta tags and og:image:alt logic unchanged.

D11 rule written above PRICES and in docs/DEPLOY.md: update PRICES.effectiveDate and PRICES.label together at Gate A and on every price edit, then rebuild dist. Date and label values unchanged. D1, D4 and D9 kept as is. D6: BABYSHOWER_LEAD_EMAIL goes in the private vendercrm-config.babyshower.php above public_html; SITE.email stays empty (host-side action for Anton). D7: mixed design in docs/design-mix-home governs. D10 still open.

Validation: node docs/qa-tools/og.mjs PASS; node build-site.mjs PASS (32 routes, 34 HTML); node verify.mjs --final PASS; git diff --check PASS; UTF-8 without BOM and LF audit PASS. Initial rg lookup FAIL (CommandNotFoundException); continued with PowerShell. Existing font/email/hosted-check flags remain. No dependencies or commit; two preexisting untracked plan prompts untouched.

## 2026-09-21 - Batch 8 - hero carousel and nine new images

Worker: none. Anton asked for the remaining work to be done in the Sonnet manager session instead of Codex (saves Codex usage); the manager wrote the code and audited it.

Images (Higgsfield, gpt_image_2_5 Sunburst, quality medium, one generate_image_batch): two 2k 16:9 heroes (hero-revelacion-genero, hero-primer-anito), five 1k 4:3 guide images (idea-*), two 1k theme retries with element-only prompts (tema-mickey-bebe, tema-blanca-nieves-bebe; both now render, no character silhouette). Preflight 1.5 credits at 2k and 1 at 1k; ledger shows nine spend lines all named GPT Image 2.5 Sunburst, total 10 credits; result sizes 2688x1520 and 1168x880 match the requested classes. Converted with webimg (--name and --alt, AVIF and WebP, heroes 640/1280/1920, cards 640/1168). Skipped on purpose: Kit Sorpresa and Revelacion Completa card images (packages are provisional until Anton finds a partner), five zone images. Manifest updated (job ids, urls, alt text, notes.batch8).

Home hero carousel (Anton's request): three slides, hero-baby-shower-quincho, hero-revelacion-genero, hero-primer-anito, behind the fixed hero text and scrim. Slide 1 is the eager LCP image and is all a no-JS visitor sees; slides 2 and 3 start hidden and site.js unhides them after the load event, so they never compete with the LCP request. Autoplay every 6 s with a 0.9 s crossfade, a pause button and three 44 px dots (strings from UI.heroSlides in content.mjs, passed through site-config), no autoplay under prefers-reduced-motion, paused while the tab is hidden. The two new heroes also head /revelacion-de-genero/ and /primer-anito/ (16:9, eager). The five guides get their image after the intro paragraph, restoring the BUILD-SPEC section 6 image slot that Batch 4 had removed; the existing verify assertion (no empty data-image-slot) still holds.

Validation by the manager: node build-site.mjs PASS (32 routes, 34 HTML); node verify.mjs --final PASS (guides 789 to 838 words); verify --final --http against the 4185 preview PASS; zip rebuilt, 141 entries, 32 routes. Real Chrome (Playwright): slide state, 6 s rotation, pause, dot selection, 44 px controls, reduced-motion (no rotation) and zero page errors; screenshots of every slide at 1440 and 375, the reveal and anito heroes, one guide and Mickey at both widths. Tap-target audit: only the known breadcrumb width and checkbox flags. Lighthouse mobile home, back to back against 814d53d on the same laptop: old 91, 86, 87 and new 91, 85, 85 (slow mode, FCP 2.6 to 2.8 s in both; LCP and CLS equal); a11y, best practices and SEO 100 on home, reveal and a guide. Transfer +115 KB on the home after load (slides 2 and 3), off the critical path.

REVISAR ANTON: the new slides show a black balloon with hands (reveal) and a dinosaur setup with a number-one topper (anito) on the home hero of a site that sells Combo Estrella first; say if you prefer the second and third slide to be a sweet table and a theme photo instead. css-audit now lists .theme-palette as unused (all ten themes have photos); kept as a fallback.

## 2026-09-21 - Batch 9 - PHP tested, local fonts, photo captions removed

Worker: none (Sonnet manager session, at Anton's request to save Codex).

PHP: portable PHP 8.3.33 NTS x64 installed at C:devphp (official windows.php.net zip, SHA-256 checked against releases.json; same minor as the Hostinger default 8.3). php -l passes. New docs/qa-tools/php-endpoint.mjs runs the real lead-forward.php under php -S with a private config one level above the document root and a fake SMTP sink (ports 8091 and 2525): 14 checks PASS, covering valid submit and redirect, phone normalisation to 595981234567, server-generated BS id on the no-JS path, one durable log line, retry with the same SID and data (no second line or email), same SID with different data rejected, invalid phone, honeypot, unknown zone, no recipient configured (503, lead still logged), rate limit (6th request 429) and GET redirect. Not tested: the VenderCRM call (needs an HTTPS endpoint) and real mail delivery on the host.

Defect found and fixed by the test: the notification mail() had no From header, which on shared hosting usually means spam or rejection. lead-forward.php now sends From: Baby Shower <no-reply@HOST> with HOST taken from the generated site defaults (babyshower.com.py); the sink confirms the header. The rest of the file is untouched.

Fonts: Instrument Serif 400 and DM Sans 400/500/700 (latin subset woff2 from Fontsource, SIL OFL, licences kept in assets/fonts) are now self-hosted. build-site.mjs prefers Satoshi if ever shipped, otherwise DM Sans; it preloads the serif and DM Sans 400, drops every Google Fonts request, and keeps the font-fallback class only as the variable switch to DM Sans. Browser check: four faces loaded from our own server, zero external requests on four pages. Lighthouse mobile home, back to back on the same laptop against the previous commit (Google Fonts): 88, 88, 89 before and 96, 96, 96 after; FCP 2.6 s to 1.1 s, LCP 3.2 s to 2.6 s, CLS 0.03 to 0. The 85 to 91 "slow mode" seen earlier on this laptop was mostly the Google Fonts request.

Captions: Anton asked to remove the illustrative text on photos. Removed the figcaption Imagen ilustrativa from every image, the appended sentence Las imágenes son ilustrativas. (UI.illustrativeImages deleted) on the home themes section and /tematicas/, the six dead figcaption CSS rules, and the caption line on og.jpg (which now uses the brand fonts). Kept on purpose, as they are not on photos and are honest answers: the FAQ item about photos of previous events ("Todavía no tenemos galería de trabajos propios. Te compartimos ideas ilustrativas por WhatsApp."), the same sentence as the intro paragraph of the themes sections, and the sentence in the package inclusions copy that says the illustrative images are ambience ideas, not own work. REVISAR ANTON: say if those three should go too; they are the only disclosure left that the photos are not real events.

Validation: node build-site.mjs PASS; node verify.mjs --final PASS and with --http against the 4185 preview PASS; zip rebuilt (147 entries, fonts included, nothing private). The Google Fonts flags no longer appear; only the two email flags remain (Anton, host side).

## 2026-09-21 - Batch 10 - last honesty sentences removed, private config prepared

Worker: none (Sonnet manager session). Anton, in chat, told the manager to skip (leave out) the remaining places that say the photos are illustrative. This changes BUILD-SPEC section 0 (gallery answer) with his explicit approval: FAQ item Puedo ver fotos de eventos anteriores? deleted from content.mjs FAQ (13 items left; the home FAQ uses slices 0-6 and is unchanged), the theme intro paragraph on the home themes section and on /tematicas/ removed (it was that FAQ answer), and the sentence Las imagenes ilustrativas son ideas de ambientacion; no son una galeria de trabajos propios removed from the Que incluye la propuesta paragraph. build-site.mjs no longer imports FAQ. No built page contains trabajos propios or ilustrativ any more; the design reference files under docs/ still do and are not shipped. REVISAR ANTON: with no disclosure left, the photos read as the studio's own work; keep the honest wording in WhatsApp answers and replace them with real photos as soon as they exist.
...
## 2026-09-24 - Batch 12 - Hostinger Git deploy readiness, steps link

Worker: none (cloud Claude Code session). Git deploy clones the whole repo into public_html; on a local Apache with a git clone as docroot, /.git/config, /plan/, /docs/, /codex-input/, /BRIEF.md, /AGENTS.md, /.claude/ and /deploy/make-zip.ps1 answered 200. .htaccess now denies dotfiles and dot-dirs (except .well-known), plan, docs, codex-input, deploy, dist, node_modules and *.md, *.mjs, *.ps1 with 403 and the 404 page body; the rules sit after the HTTPS/apex redirect. Re-probed: all site routes, assets, robots, sitemap and .well-known 200; every private path 403; http and www redirect to https apex; HTML no-store, assets immutable, gzip on. verify.mjs asserts the new rules. Steps in docs/HOSTINGER-GIT.md.
