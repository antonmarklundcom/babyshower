...
import { ADDONS, REVEAL, ANITO, COMPARISON, EXTRA_FOOD, CONFIRMATION, ZERO_PRICE, extraGuestCaption } from './content.mjs';
...
function cards(route, items = PACKAGES, delivery = 0) {
 return `<div class="package-grid">${items.map(item => ({ ...item, price: item.price + delivery })).map((item, i) => `<article class="card ${i === 1 ? 'card--raised featured' : i === 0 ? 'card--hair' : 'card--bare'}" data-package="${item.id}" data-price="${item.price}">${route === '/' ? '<div class="package-summary">' : ''}${(['/', '/combos-y-precios/'].includes(route) ? imageFigure(imagery.find(image => image.use === 'paquete ' + item.name.replace('Combo ', ''))?.id, 'package-image', imageSizes.card) : '') || `<div class="motif motif-${i}" aria-hidden="true"><span></span></div>`}${item.badge ? `<span class="badge">${esc(item.badge)}</span>` : ''}<h3>${esc(item.name)}</h3><p class="price-caption" data-price-caption>${esc(priceCaption(item.price))}</p>${route === '/' ? `<div class="package-description">${p(item.description)}</div><div class="package-chip">${p(item.chip)}</div></div><dl class="package-rows">${COMPARISON.map(row => `<div><dt>${esc(row[0])}</dt><dd>${esc(row[i + 1])}</dd></div>`).join('')}</dl>` : p(item.description) + p(item.chip)}<details><summary>${esc(UI.exclusions)}</summary>${p(item.exclusions)}</details>${link(waHref(packageMessage(item, route)), PRIMARY_CTA, 'combo-' + item.id, 'btn btn--primary')}</article>`).join('')}</div>`;
}
...
function offerCard(item, route, heading = 'h2') {
 return `<article class="card card--hair" data-offer="${item.id}" data-price="${item.price}"><${heading}>${esc(item.name)}</${heading}><p data-price-caption>${esc(priceCaption(item.price))}</p>${item.description ? p(item.description) : ''}${item.chip ? p(item.chip) : ''}${link(waHref(packageMessage(item, route)), PRIMARY_CTA, item.id, 'btn btn--primary')}</article>`;
}
...
function themeGrid(items, cls = 'theme-bento', images = false) {
 ...
 const media = image || (images ? `<div class="theme-image theme-palette" aria-hidden="true">${t.swatches.map(color => `<span style="background-color:${esc(color)}"></span>`).join('')}</div>` : '');
...
function body(route, page) {
 ...
 const hero = `<section class="inner-hero${page.type === 'theme' ? ' tint-' + page.detail.tint : ''}"><div class="wrap"><span class="eyebrow">${esc(SITE.name)}</span><h1>${esc(page.h1)}</h1>${page.type === 'theme' ? imageFigure(imagery.find(image => image.use === route)?.id, 'theme-hero-image', imageSizes.theme) : ''}</div></section>`;
