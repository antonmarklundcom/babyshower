...
function homeThemes() {
 return `<div class="theme-grid">${THEME_DETAILS.map(t => {
...
function home(route, page) {
 ...
 <section class="steps-section"><div class="wrap"><span class="eyebrow">${esc(UI.steps)}</span><h2>${esc(UI.homeSteps)}</h2><ol class="steps">${STEPS.map(([title, body], i) => `<li><span class="step-number" aria-hidden="true">0${i + 1}</span><h3>${esc(title)}</h3>${p(body)}</li>`).join('')}</ol></div></section>
 <section class="theme-section"><div class="wrap"><div class="theme-heading"><h2>${esc(UI.themes)}</h2>${p(FAQ[12].a + ' ' + UI.illustrativeImages)}</div>${homeThemes()}${link('/tematicas/', UI.themesLink, 'tematicas', 'home-more')}</div></section>
