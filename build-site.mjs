...
const ASSET_V = Object.fromEntries(['assets/css/site.css', 'assets/js/site.js', 'assets/js/motion.js'].map(path => [path, hash(path)]));
...
function graph(route, page) {
 ...
 if (page.type === 'home') data.push({ '@type': 'Service', '@id': SITE.url + route + '#servicio', name: page.h1, provider: { '@id': org }, areaServed: SITE.area, serviceType: UI.eyebrow });
