...
import { SERVICES, SERVICE_NAV, SERVICE_HUB, SERVICE_NOTE, FOOTER_GUIDES } from './content.mjs';
...
for (const t of THEME_DETAILS) PAGES[`/tematicas/${t.slug}/`] = { type: 'theme', detail: t, title: `Decoración ${t.character ? 'inspirada en' : 'de'} ${t.name} para baby shower`, description: `Explorá la propuesta de ${t.name} para baby shower o primer añito: colores, elementos y combos con precios estimados. Consultá por WhatsApp.`, h1: `Decoración ${t.character ? 'inspirada en' : 'de'} ${t.name} para baby shower y primer añito`, faq: t.faq };
for (const z of ZONE_DETAILS.filter(z => manifest.some(r => r.route === `/zonas/${z.slug}/`))) {
 ...
 PAGES[`/zonas/${z.slug}/`] = { type: 'zone', detail: z, delivery: delivery.delivery, title: `Baby shower en ${z.name} | Decoración y combos`, description: `Consultá tu baby shower en ${z.name}. Conocé los espacios, el recargo estimado y los combos con traslado ya sumado al precio mostrado.`, h1: `Baby shower en ${z.name}`, faq: z.faq.map((f, i) => i ? f : { ...f, a: `${deliveryNote(delivery)} ${f.a}` }) };
}
...
function header(route) {
 ...
 const mobile = `<p class="mobile-nav__label">Servicios</p><ul class="mobile-nav__sub">${SERVICE_NAV.map(([href, label]) => `<li>${link(href, label, 'menu-movil', '', current(href, route))}</li>`).join('')}</ul>${nav(headerNav.filter(([href]) => href !== '/servicios/'), route, 'menu-movil')}`;
 return `<a class="skip" href="#contenido">${esc(UI.skip)}</a><header class="hdr" data-hdr><div class="wrap hdr-row"><a href="/" class="brand" data-ev="navigation" data-ev-loc="marca">${esc(SITE.brand)}<span class="brand-suffix">${esc(SITE.suffix)}</span></a><nav class="desktop-nav" aria-label="${esc(UI.mainNav)}">${desktop}</nav><div class="header-contact"><span class="phone">${esc(SITE.phone)}</span>${wa(route, 'cabecera', 'btn btn--outline', true)}</div><button type="button" class="burger" data-hdr-burger aria-expanded="false" aria-controls="hdr-panel" aria-label="${esc(UI.menu)}" data-ev="menu_open" data-ev-loc="cabecera"><svg viewBox="0 0 24 24" aria-hidden="true"><path d="M3 6h18M3 12h18M3 18h18" stroke="currentColor" stroke-width="2"/></svg></button></div><nav id="hdr-panel" data-hdr-panel class="mobile-nav wrap" aria-label="${esc(UI.mainNav)}">${mobile}</nav></header>`;
}
function footer(route) {
 const col = (title, items) => `<nav class="footer-col" aria-label="${esc(title)}"><h2>${esc(title)}</h2><ul>${items.map(([href, label]) => `<li>${link(href, label, 'pie', '', current(href, route))}</li>`).join('')}</ul></nav>`;
 const zoneItems = ZONE_DETAILS.filter(z => manifest.some(r => r.route === `/zonas/${z.slug}/`)).map(z => [`/zonas/${z.slug}/`, z.name]);
 const guideItems = FOOTER_GUIDES.map(slug => guides.find(g => g.route === `/ideas/${slug}/`)).filter(Boolean).map(g => [g.route, g.detail.title.split(/[:|]/)[0].trim()]);
...
const imageSizes = {
 ...
 guide: '(max-width: 959px) 90vw, 720px'
};
...
function cards(route, items = PACKAGES, delivery = 0) {
 return '<div class="package-grid">' + items.map(item => offerCard({ ...item, basePrice: item.price, price: item.price + delivery }, route, 'h3')).join('') + '</div>';
}
const stepList = (items = STEPS) => '<ol class="steps" style="--step-count:' + items.length + '">' + items.map((item, i) => '<li><span class="step-number" aria-hidden="true">0' + (i + 1) + '</span>' + (Array.isArray(item) ? '<h3>' + esc(item[0]) + '</h3>' + p(item[1]) : p(item)) + '</li>').join('') + '</ol>';
const steps = () => '<section class="steps-section"><div class="wrap"><h2>' + esc(UI.steps) + '</h2>' + stepList() + '</div></section>';
...
function homeCards(route) {
 return `<div class="package-grid">${PACKAGES.map((item, i) => `<article class="card${i === 1 ? ' featured' : ''}" data-package="${item.id}" data-price="${item.price}">
  <div class="package-heading"><h3>${esc(item.name)}</h3><span class="package-tag${item.badge ? ' badge' : ''}">${esc(UI.homePackageTags[i])}</span></div>
  ${imageFigure(['combo-nube', 'combo-estrella', 'combo-sueno'][i], 'package-image', '(max-width: 959px) calc(100vw - 32px), (max-width: 1439px) 30vw, 395px')}
  <div class="package-body"><p class="price-caption" data-price-caption><span class="price-label">${esc(UI.pricePrefix)}</span> <span class="price-amount">${esc(fmtGs(item.price))}</span></p>
  <div class="package-description">${p(item.description)}</div><div class="package-chip">${p(item.chip)}</div>
  <dl class="package-rows">${[0, 3, 4, 5, 8, 9].map(index => COMPARISON[index]).map(row => `<div><dt>${esc(row[0])}</dt><dd>${esc(row[i + 1])}</dd></div>`).join('')}</dl>
  <details><summary>${esc(UI.exclusions)}</summary>${p(i === 0 ? EXCLUSIONS : item.exclusions)}</details>
  <div class="package-action">${link(waHref(packageMessage(item, route)), PRIMARY_CTA, 'combo-' + item.id, i === 1 ? 'btn btn--primary' : 'btn btn--outline')}<p class="package-confirmation">${esc(CONFIRMATION)}</p></div></div>
 </article>`).join('')}</div>`;
}
function homeInclusions() {
 return `<div class="home-inclusions">${UI.homeInclusions.map(group => `<div><h2 class="eyebrow">${esc(group.title)}</h2><ul>${group.items.map(item => `<li>${esc(item)}</li>`).join('')}</ul></div>`).join('')}</div>`;
}
function themeGrid(items = THEME_DETAILS, cls = 'theme-grid') {
 return `<div class="theme-grid ${cls}">${items.map(t => {
  const entry = imagery.find(image => image.use === `/tematicas/${t.slug}/` && image.placed && image.status !== 'rejected');
  const photo = imageFigure(entry?.id, 'theme-image', '(max-width: 959px) calc((100vw - 48px) / 2), (max-width: 1439px) 18vw, 235px');
  return `<a class="theme-card tint-${t.tint}${photo ? ' theme-photo' : ' theme-swatch'}" href="/tematicas/${t.slug}/" data-ev="navigation" data-ev-loc="tematicas">${photo || themePalette(t)}<span class="theme-name">${esc(t.name)}${photo ? '' : `<small>${esc(UI.palette)}: ${esc(t.palette)}</small>`}</span></a>`;
 }).join('')}</div>`;
}
function homeZones() {
...
function heroSlides() {
 const slides = HERO_SLIDES.map((id, i) => {
  const figure = imageFigure(id, 'hero-art hero-slide' + (i ? '' : ' is-active'), '100vw', i === 0);
  return i ? figure.replace('<figure ', '<figure hidden aria-hidden="true" ') : figure;
 }).join('');
 return `<div class="hero-slides" data-hero-slides role="group" aria-roledescription="carousel" aria-label="${esc(UI.heroSlides.label)}">${slides}</div>`;
}
function home(route, page) {
 return `<section class="hero grain">${heroSlides()}<div class="wrap"><div class="hero-copy"><span class="eyebrow">${esc(UI.eyebrow)}</span><h1>${esc(page.h1)}</h1>${p(UI.hero[0])}<div class="hero-price"><span class="price-label">${esc(UI.pricePrefix)}</span><span class="price-amount">${esc(fmtGs(PACKAGES[1].price))}</span></div><p class="hero-confirmation">${esc(CONFIRMATION)} <span class="hero-desktop">${esc(UI.hero[2])}</span></p><div class="actions">${wa(route, 'portada')}${link('#calculadora', UI.calc, 'portada', 'btn btn--ghost')}</div></div></div></section>
 <div class="trust-ribbon"><div class="wrap"><ul>${TRUST.map(t => `<li>${esc(t)}</li>`).join('')}</ul></div></div>
...
function calculator(route) {
 ...
 return `<section class="calc-section" id="calculadora"><div class="wrap">${route === '/' ? '' : `<span class="eyebrow">${esc(UI.calculator)}</span><h2>${esc(UI.calcTitle)}</h2>`}${link('/combos-y-precios/', UI.prices, 'calculadora-sin-js')}<div class="calc-panel" data-calculator hidden><script type="application/json" data-calc-config>${json(config)}</script><div>${route === '/' ? `<span class="eyebrow">${esc(UI.calculator)}</span><h2>${esc(UI.calcTitle)}</h2>` : ''}<fieldset><legend>Elegí tu combo</legend>${PACKAGES.map(item => `<label class="calc-choice"><input type="radio" name="calc-package" value="${item.id}"${item.id === 'estrella' ? ' checked' : ''}><span>${esc(item.name)}<small>${esc(priceCaption(item.price))}</small></span></label>`).join('')}</fieldset><fieldset><legend>Invitados: <span data-guest-count>30</span></legend><div class="guest-controls"><button type="button" data-guest-step="-5" aria-label="Restá 5 invitados" data-ev="calc_adjust" data-ev-loc="calculadora">−</button><label class="sr-only" for="calc-guests">Cantidad de invitados</label><input id="calc-guests" data-guests type="range" min="15" max="60" step="5" value="30"><button type="button" data-guest-step="5" aria-label="Sumá 5 invitados" data-ev="calc_adjust" data-ev-loc="calculadora">+</button></div></fieldset><fieldset><legend>Adicionales</legend>${ADDONS.map(a => `<label class="calc-choice"><input type="checkbox" data-addon value="${a.id}"><span>${esc(a.name)} <strong data-addon-note="${a.id}"></strong><small>${esc(priceCaption(a.price))}</small></span></label>`).join('')}</fieldset><label for="calc-zone">Zona</label><select id="calc-zone" data-calc-zone>${ZONES.map(z => `<option value="${z.slug}">${esc(z.name)} — ${z.delivery === null ? 'cotizar' : z.delivery === 0 ? esc(ZERO_PRICE) : 'recargo estimado Gs. ' + new Intl.NumberFormat('es-PY').format(z.delivery)}</option>`).join('')}</select>${p(DELIVERY)}</div><div>${route === '/' ? `<h3 class="eyebrow">${esc(UI.estimation)}</h3>` : ''}<div data-calc-output aria-live="polite" aria-atomic="true"></div>${link(waDefault(route), PRIMARY_CTA, 'calculadora', 'btn btn--primary', 'data-calc-cta')}</div></div></div></section>`;
}
function offerCard(item, route, heading = 'h2') {
 const index = PACKAGES.findIndex(pkg => pkg.id === item.id);
 const featured = item.id === 'estrella';
 return `<article class="card offer-card${featured ? ' featured' : ''}" data-${index >= 0 ? 'package' : 'offer'}="${item.id}" data-price="${item.price}"><div class="package-heading"><${heading}>${esc(item.name)}</${heading}>${item.badge ? '<span class="package-tag badge">' + esc(item.badge) + '</span>' : ''}</div>${route === '/combos-y-precios/' && index >= 0 ? imageFigure(['combo-nube', 'combo-estrella', 'combo-sueno'][index], 'package-image', imageSizes.card) : ''}<div class="package-body"><p class="price-caption" data-price-caption><span class="price-label">${esc(UI.pricePrefix)}</span> <span class="price-amount">${esc(fmtGs(item.price))}</span></p>${route.startsWith('/zonas/') && item.price > item.basePrice ? p(UI.zonePriceParts(item.basePrice, item.price - item.basePrice)) : ''}${item.description ? '<div class="package-description">' + p(item.description) + '</div>' : ''}${item.chip ? '<div class="package-chip">' + p(item.chip) + '</div>' : ''}${index >= 0 || [ANITO.id, ...REVEAL.packages.map(offer => offer.id), ...SERVICES.flatMap(s => s.offers.map(offer => offer.id))].includes(item.id) ? '<details><summary>' + esc(UI.exclusions) + '</summary>' + p(item.exclusions || EXCLUSIONS) + '</details>' : ''}<div class="package-action">${link(waHref(packageMessage(item, route)), PRIMARY_CTA, index >= 0 ? 'combo-' + item.id : item.id, featured ? 'btn btn--primary' : 'btn btn--outline')}<p class="package-confirmation">${esc(CONFIRMATION)}</p></div></div></article>`;
}
function offers(route, page) {
 ...
 if (page.type === 'reveal') return `<section><div class="wrap">${p(REVEAL.intro)}<div class="offer-grid">${REVEAL.packages.map(a => offerCard(a, route)).join('')}</div><h2>Cómo funciona</h2>${stepList(REVEAL.steps)}${offerCard(REVEAL.cake, route)}<h2>Adicionales a confirmar, solo exterior</h2><ul><li>Humo de color</li><li>Polvo holi</li></ul><h2>Revelación sencilla en casa</h2>${link('/ideas/revelacion-de-genero-sencilla/', '¿Querés hacerlo simple en casa? Mirá ideas de revelación de género sencilla', 'ideas-revelacion')}${p(REVEAL.repeat)}</div></section>${questions}`;
 return `<section><div class="wrap">${offerCard(ANITO, route)}<h2>Elegí tu temática</h2>${themeGrid(THEME_DETAILS.filter(t => ANITO.themes.includes(t.slug)))}</div></section>${questions}`;
}
...
function ideasBody(route, page) {
 ...
 return `<article class="guide wrap" data-guide><nav class="guide-breadcrumb" aria-label="Ruta de navegación">${link('/', 'Inicio', 'migas')} / ${link('/ideas/', 'Ideas', 'migas')}</nav>${p(idea.intro)}${imageFigure(imagery.find(image => image.use === route && image.placed && image.status !== 'rejected')?.id, 'guide-figure', imageSizes.guide)}${idea.sections.map((s, i) => `<section class="guide-section"><h2>${esc(s.heading)}</h2>${s.paragraphs.map(p).join('')}</section>${i === 1 ? guideCta(route, idea, 'guia-intermedia') : ''}`).join('')}<div class="guide-related"><p>Explorá la temática ${link(`/tematicas/${theme.slug}/`, theme.name, 'guia-tematica')} y compará ${link('/combos-y-precios/', 'combos y precios', 'guia-combos')} para preparar tu consulta.</p>${idea.offer.id === 'kit' ? link('/revelacion-de-genero/', 'Conocé el Kit Sorpresa', 'guia-kit') : ''}${(idea.links || []).map(([href, label]) => `<p>${link(href, label, 'guia-enlace')}</p>`).join('')}</div>${guideCta(route, idea, 'guia-final')}</article>`;
}
...
// Only opt into self-hosted fonts when the required faces are all present. Text face: Satoshi if shipped, otherwise DM Sans (both need 400, 500 and 700).
...
const textFace = [['Satoshi', 'satoshi'], ['DM Sans', 'dm-?sans']].map(([family, pattern]) => ({ family, files: [400, 500, 700].map(w => fontFiles.find(f => new RegExp(`${pattern}.*(${w}|${{400:'regular',500:'medium',700:'bold'}[w]})`, 'i').test(f))) })).find(face => face.files.every(Boolean));
const localFonts = serif && textFace;
const fontUrl = f => '/assets/fonts/' + encodeURIComponent(f);
const fonts = localFonts ? [serif, textFace.files[0]].map(f => `<link rel="preload" href="${fontUrl(f)}" as="font" type="font/woff2" crossorigin>`).join('') + `<style>${[[serif, 'Instrument Serif', 400], ...textFace.files.map((f, i) => [f, textFace.family, [400, 500, 700][i]])].map(([f, family, weight]) => `@font-face{font-family:'${family}';font-style:normal;font-weight:${weight};font-display:swap;src:url('${fontUrl(f)}') format('woff2')}`).join('')}</style>` : '<link rel="preconnect" href="https://fonts.googleapis.com"><link rel="preconnect" href="https://fonts.gstatic.com" crossorigin><link href="https://fonts.googleapis.com/css2?family=DM+Sans:wght@400;500;700&amp;family=Instrument+Serif&amp;display=swap" rel="preload" as="style" onload="this.onload=null;this.rel=&quot;stylesheet&quot;"><noscript><link href="https://fonts.googleapis.com/css2?family=DM+Sans:wght@400;500;700&amp;family=Instrument+Serif&amp;display=swap" rel="stylesheet"></noscript>';
